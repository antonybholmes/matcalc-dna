...
public class DnaDialog extends ModernDialogHelpWindow {
	...
	private ModernCheckSwitch mCheckRevComp = 
			new ModernCheckSwitch("Reverse complement");
	
	private ModernCompactSpinner mExt5pSpinner =
			new ModernCompactSpinner(0, 10000, 0);
	
	private ModernCompactSpinner mExt3pSpinner =
			new ModernCompactSpinner(0, 10000, 0);
	
	private ModernRadioButton mCheckUpper = 
			new ModernRadioButton("UPPERCASE");
	
	private ModernRadioButton mCheckLower = 
			new ModernRadioButton("lowecase");
	...
	public void clicked(ModernClickEvent e) {
		if (e.getMessage().equals(UI.BUTTON_OK)) {
			...
			SettingsService.getInstance().update("org.matcalc.toolbox.bio.dna.from-center", 
